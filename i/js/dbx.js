...
function dbxGroup() {
    ...
    this.vocab = {'open': args[7], 'close': args[8], 'move': args[9], 'toggle': args[10], 'kmove': args[11], 'ktoggle': args[12], 'syntax': args[13], 'kyes': (typeof args[14] != 'undefined' ? args[14] : ''), 'kno': (typeof args[15] != 'undefined' ? args[15] : '')};
    var self = this;
    this.dragok = false;
    this.box = null;
    this.dialog = null;
    this.buffer = null;
    this.last = {'box': null, 'direction': null};
    this.child = {'first': null, 'last': null};
    this.keytimer = null;
...
dbxGroup.prototype.moveBoxByKeyboard = function (e, anchor, parent, direction, confirm, manual) {
    ...
    var clonepoint = {'x': parent.offsetLeft, 'y': parent.offsetTop};
    var differences = [];
    ...
    for (var i in boxes) {
        ...
        var boxpoint = {'x': boxes[i].offsetLeft, 'y': boxes[i].offsetTop};
        differences.push([i, boxpoint.x - clonepoint.x, boxpoint.y - clonepoint.y]);
        if (parent == boxes[i]) {
            index = i;
        }
    }
    var splitdiffs = {'positive': [], 'negative': []};
    ...
    if (typeof dbx.onboxdrag == 'undefined' || dbx.onboxdrag()) {
        if (box != targetbox && boxes == this.boxes) {
            var origpoint = {'x': box.offsetLeft + (box.offsetWidth / 2), 'y': box.offsetTop + (box.offsetHeight / 2)};
            var boxpoint = {'x': targetbox.offsetLeft + (targetbox.offsetWidth / 2), 'y': targetbox.offsetTop + (targetbox.offsetHeight / 2)};
        ...
        if (confirm || this.dialog) {
            ...
            if (boxes != this.boxes) {
                ...
                var diffs = {'x': groupcontainer.left - callcontainer.left, 'y': groupcontainer.top - callcontainer.top};
            }
...
dbxGroup.prototype.insertTwoBoxes = function (original, selected, anchor, manual, positive) {
    ...
    for (i = 0; i < visiboxes.length; i++) {
        visiposes.push({'x': visiboxes[i].offsetLeft, 'y': visiboxes[i].offsetTop});
    }
        ...
        if (this.released || overall.x > this.threshold || overall.x < (0 - this.threshold) || overall.y > this.threshold || overall.y < (0 - this.threshold)) {
            ...
            if (typeof dbx.onboxdrag == 'undefined' || dbx.onboxdrag()) {
                ...
                if (this.restrict == 'freeform') {
                    var clonepoint = {'x': this.boxclone.offsetLeft + (this.boxclone.offsetWidth / 2), 'y': this.boxclone.offsetTop + (this.boxclone.offsetHeight / 2)};
                    var proportion = 0.2;
...
dbxGroup.prototype.updateChildClasses = function () {
    ...
    var children = {'first': boxids[0], 'last': boxids[boxids.length - 1]};
...
dbxGroup.prototype.moveBoxByMouse = function (clientX, clientY, confirm) {
    if (this.orientation == 'freeform') {
        var clonepoint = {'x': clientX - this.difference.x + (this.boxclone.offsetWidth / 2), 'y': clientY - this.difference.y + (this.boxclone.offsetHeight / 2)};
        var differences = [];
    } else {
        var cloneprops = {'xy': this.orientation == 'vertical' ? clientY - this.difference.y : clientX - this.difference.x, 'wh': this.orientation == 'vertical' ? this.boxclone.offsetHeight : this.boxclone.offsetWidth};
    }
    if (dbx.hide) {
    ...
    for (var i in this.boxes) {
        ...
        if (this.orientation == 'freeform') {
            ...
            var boxpoint = {'x': this.boxes[i].offsetLeft + (this.boxes[i].offsetWidth / 2), 'y': this.boxes[i].offsetTop + (this.boxes[i].offsetHeight / 2)};
            ...
            var boxprops = {'xy': this.orientation == 'vertical' ? this.boxes[i].offsetTop : this.boxes[i].offsetLeft, 'wh': this.orientation == 'vertical' ? this.boxes[i].offsetHeight : this.boxes[i].offsetWidth};
    ...
    if (this.orientation == 'freeform') {
        ...
        boxprops = {'left': originaltargetbox.offsetLeft, 'top': originaltargetbox.offsetTop};
        ...
        var origpoint = {'x': this.box.offsetLeft + (this.box.offsetWidth / 2), 'y': this.box.offsetTop + (this.box.offsetHeight / 2)};
        boxpoint = {'x': originaltargetbox.offsetLeft + (originaltargetbox.offsetWidth / 2), 'y': originaltargetbox.offsetTop + (originaltargetbox.offsetHeight / 2)};
        ...
        this.last = {'box': originaltargetbox, 'direction': this.direction};
    } else {
        var targetbox = this.boxes[index];
        var originaltargetbox = targetbox;
    }
...
function dbxAnimator(caller, box, pos, res, kbd, anchor, manual) {
    ...
    var after = {'x': this.box.offsetLeft, 'y': this.box.offsetTop};
    if (!(before.x == after.x && before.y == after.y)) {
        ...
        this.box.style.visibility = 'hidden';
        var change = {'x': after.x > before.x ? after.x - before.x : 0 - (before.x - after.x), 'y': after.y > before.y ? after.y - before.y : 0 - (before.y - after.y)};
